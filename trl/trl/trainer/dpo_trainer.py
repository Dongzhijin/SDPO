...
class DPOTrainer(Trainer):
    ...
    def compute_ref_log_probs(self, batch: dict[str, torch.LongTensor]) -> dict:
        ...
        # For SDPO, also return per-token logps if available
        if self.loss_type == "sdpo" and "per_token_logps" in ref_model_output:
            return (
                ref_model_output["chosen_logps"], 
                ref_model_output["rejected_logps"],
                ref_model_output["per_token_logps"]
            )
        else:
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
        ...
        elif self.loss_type == "sdpo":
            # SDPO loss: same as sigmoid but with importance-based token masking
            # The masking is handled in get_batch_loss_metrics before calling this function
            losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing
            )
            ...
            raise ValueError(
                ...
                "'nca_pair', 'robust', 'bco_pair', 'sppo_hard', 'aot', 'aot_pair', 'discopop', 'apo_zero', 'apo_down', 'sdpo']"
            )
    ...
    def concatenated_forward(
        ...
        # For SDPO, we need to store per-token logps for later masking
        # This applies to both policy and reference models
        if self.loss_type == "sdpo":
            # Store per-token logps for importance-based masking
            # Remove the first token (shift for autoregressive prediction)
            output["per_token_logps"] = per_token_logps[:, 1:]
    ...
    def mask_unimportant_tokens(
        ...
        """
        Mask unimportant tokens for SDPO loss based on importance scores.
        
        Args:
            chosen_per_token_logps: Per-token log probabilities for chosen responses
            rejected_per_token_logps: Per-token log probabilities for rejected responses  
            ref_chosen_per_token_logps: Reference model per-token log probabilities for chosen responses
            ref_rejected_per_token_logps: Reference model per-token log probabilities for rejected responses
            threshold: Quantile threshold for importance masking (default: 0.6)
            
        Returns:
            Tuple of masked and summed log probabilities: (chosen_logps, rejected_logps, ref_chosen_logps, ref_rejected_logps)
        """
        # Calculate importance scores: larger values indicate more important tokens
        chosen_per_token_important_scores = ref_chosen_per_token_logps - chosen_per_token_logps
        rejected_per_token_important_scores = rejected_per_token_logps - ref_rejected_per_token_logps
        
        # Create a copy to avoid modifying original tensors
        chosen_logps_masked = chosen_per_token_logps.clone()
        rejected_logps_masked = rejected_per_token_logps.clone()
        ref_chosen_logps_masked = ref_chosen_per_token_logps.clone()
        ref_rejected_logps_masked = ref_rejected_per_token_logps.clone()
        
        # Set importance scores for zero log-prob tokens to negative infinity (will be excluded)
        chosen_per_token_important_scores = chosen_per_token_important_scores.clone()
        rejected_per_token_important_scores = rejected_per_token_important_scores.clone()
        chosen_per_token_important_scores[chosen_per_token_logps == 0] = float('-inf')
        rejected_per_token_important_scores[rejected_per_token_logps == 0] = float('-inf')
        
        # Calculate quantile thresholds for each sequence in the batch
        chosen_thresholds = []
        rejected_thresholds = []
        
        for scores in chosen_per_token_important_scores:
            valid_scores = scores[scores > float('-inf')]
            if valid_scores.numel() > 0:
                chosen_thresholds.append(torch.quantile(valid_scores, threshold))
            else:
                chosen_thresholds.append(torch.tensor(float('-inf'), device=scores.device))
        
        for scores in rejected_per_token_important_scores:
            valid_scores = scores[scores > float('-inf')]
            if valid_scores.numel() > 0:
                rejected_thresholds.append(torch.quantile(valid_scores, threshold))
            else:
                rejected_thresholds.append(torch.tensor(float('-inf'), device=scores.device))
        
        chosen_thresholds = torch.stack(chosen_thresholds)
        rejected_thresholds = torch.stack(rejected_thresholds)
        
        # Create masks for important tokens
        chosen_mask = chosen_per_token_important_scores >= chosen_thresholds.unsqueeze(1)
        rejected_mask = rejected_per_token_important_scores >= rejected_thresholds.unsqueeze(1)
        
        # Apply masks - set unimportant tokens to 0
        chosen_logps_masked[~chosen_mask] = 0
        rejected_logps_masked[~rejected_mask] = 0
        ref_chosen_logps_masked[~chosen_mask] = 0
        ref_rejected_logps_masked[~rejected_mask] = 0
        
        # Return summed log probabilities
        return (
            chosen_logps_masked.sum(-1), 
            rejected_logps_masked.sum(-1), 
            ref_chosen_logps_masked.sum(-1), 
            ref_rejected_logps_masked.sum(-1)
        )
    ...
    def get_batch_loss_metrics(
        ...
        if self.args.use_liger_loss:
            ...
            if "ref_chosen_logps" in batch and "ref_rejected_logps" in batch:
                ...
                ref_outputs = self.compute_ref_log_probs(batch)
                if self.loss_type == "sdpo" and len(ref_outputs) == 3:
                    ref_chosen_logps, ref_rejected_logps, ref_per_token_logps = ref_outputs
                else:
                    ref_chosen_logps, ref_rejected_logps = ref_outputs

            # Handle SDPO importance-based token masking
            if self.loss_type == "sdpo" and "per_token_logps" in model_output:
                # Extract per-token logps for both policy and reference models
                policy_per_token_logps = model_output["per_token_logps"]
                
                # Check if we have reference per-token logps
                if 'ref_per_token_logps' in locals():
                    # Split policy and reference per-token logps
                    num_examples = batch["prompt_input_ids"].shape[0]
                    policy_chosen_per_token = policy_per_token_logps[:num_examples]
                    policy_rejected_per_token = policy_per_token_logps[num_examples:]
                    ref_chosen_per_token = ref_per_token_logps[:num_examples]
                    ref_rejected_per_token = ref_per_token_logps[num_examples:]
                    
                    # Apply importance-based masking
                    (
                        masked_chosen_logps,
                        masked_rejected_logps,
                        masked_ref_chosen_logps,
                        masked_ref_rejected_logps,
                    ) = self.mask_unimportant_tokens(
                        policy_chosen_per_token,
                        policy_rejected_per_token,
                        ref_chosen_per_token,
                        ref_rejected_per_token,
                        threshold=self.args.sdpo_threshold,
                    )
                    
                    # Use masked logps for loss computation
                    losses, chosen_rewards, rejected_rewards = self.dpo_loss(
                        masked_chosen_logps, masked_rejected_logps, masked_ref_chosen_logps, masked_ref_rejected_logps
                    )
                else:
                    # Fallback to original logps if ref per-token logps are not available
                    losses, chosen_rewards, rejected_rewards = self.dpo_loss(
